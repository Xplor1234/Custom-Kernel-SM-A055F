...
struct gendisk *ufs_disk[SD_NUM] = {NULL, NULL, NULL};
#endif
        ...
        char *buf)
{
	...
	{
		p = &((*gd_t)->part0);
		printk("(*gd_t)->disk_name is %s, p->nr_sects = %llu, (unsigned long long)part_nr_sects_read(p) = %llu\n", (*gd_t)->disk_name, p->nr_sects, (unsigned long long)part_nr_sects_read(p));
		if ((strcmp((*gd_t)->disk_name, "sda") == 0) || (strcmp((*gd_t)->disk_name, "sdb") == 0) || (strcmp((*gd_t)->disk_name, "sdc") == 0))
	...
	if (strcmp(model_name, "eUFS2.2_128") == 0)
		return sprintf(buf, "%s_PSUF22H17-A128SS_%dGB_%dGB\n",vendor_name,
			calc_ddr_size(), calc_ufs_size(ufs_size));
	else if (strcmp(model_name, "eUFS2.2_064") == 0)
		return sprintf(buf, "%s_PSUF22H17-A064SS_%dGB_%dGB\n",vendor_name,
			calc_ddr_size(), calc_ufs_size(ufs_size));
	else
...
static ssize_t
show_ram_size(struct device *dev, struct device_attribute *attr,
        char *buf)
{
    return sprintf(buf, "%dGB\n",calc_ddr_size());
}

static DEVICE_ATTR(ram_size, S_IRUGO, show_ram_size, NULL);

static ssize_t
show_rom_size(struct device *dev, struct device_attribute *attr,
        char *buf)
{
	struct hd_struct *p = NULL;
	struct gendisk **gd_t = ufs_disk;
...
static DEVICE_ATTR(rom_size, S_IRUGO, show_rom_size, NULL);
#endif
